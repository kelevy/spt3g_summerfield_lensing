...
if args.src_yaml is None:
    ...
    dir_p    = config['lensrec']['dir_out'].format(runname=runname,rectype=rectype,lminT=lminT,lminP=lminP,lmaxT=lmaxT,lmaxP=lmaxP,mmin=mmin)
    dir_p_N1 = config['lensrec']['dir_out_N1'].format(runname=runname,rectype=rectype,lminT=lminT,lminP=lminP,lmaxT=lmaxT,lmaxP=lmaxP,mmin=mmin)
else:
    #save stacks in hardened GMV plm directories
    sconfig  = utils.load_yaml(args.src_yaml)
    dir_p    = sconfig['lensrec']['dir_out'].format(runname=runname,rectype=rectype,lminT=lminT,lminP=lminP,lmaxT=lmaxT,lmaxP=lmaxP,mmin=mmin,prftype=prftype)
    dir_p_N1 = sconfig['lensrec']['dir_out_N1'].format(runname=runname,rectype=rectype,lminT=lminT,lminP=lminP,lmaxT=lmaxT,lmaxP=lmaxP,mmin=mmin,prftype=prftype)
...
if getresp:
    ...
    for i in range(1,nsims+1):
        ...
        #np.save('cl1_summerb_seed%s' %(i), hp.alm2cl(ilm,ilm))
        l=np.arange(len(cl1))
        cl2+=hp.alm2cl(olm-mfi,ilm)*0.5*(l*(l+1))
        #np.save('cl2_summerb_seed%s' %(i), hp.alm2cl(olm-mfi,ilm)*0.5*(l*(l+1)))
        c+=1
