...
parser.add_argument("--nber_bundles", default=1, type=int, help="nber_bundles")
...
def compute_cls(config, seed, ktype="xxxx", correct_by_theory=False, bundleid1=None, bundleid2=None):
    """
    ...
    if bundleid1 is None and bundleid2 is None:
        ...
        file_1 = config["pspec"]["dir_kmaps"].format(runname=config["base"]["runname"], rectype=config["lensrec"]["rectype"]) + "bundle%sXbundle%s/kmap%s_%s_%d_1.fits" % (bundleid1, bundleid1, ktype[:2], qid, i)
        file_2 = config["pspec"]["dir_kmaps"].format(runname=config["base"]["runname"], rectype=config["lensrec"]["rectype"]) + "bundle%sXbundle%s/kmap%s_%s_%d_2.fits" % (bundleid2, bundleid2, ktype[2:], qid, i)
    file_mask = config["pspec"]["mask_analysis"]
    
    #alm1=hp.read_alm(file_1)
    #alm2=hp.read_alm(file_2)
...
def get_kmap(config, seed, ktype="xx", mapnum=1, bundleid1=None, bundleid2=None):
    """
    ...
    for qe in qes:
        if bundleid1 is None:
            dir_plm = config["lensrec"]["dir_out_N1"].format(runname=config["base"]["runname"], rectype=config["lensrec"]["rectype"])
        else:
            dir_plm = config["lensrec"]["dir_out"].format(runname=config["base"]["runname"], rectype=config["lensrec"]["rectype"], bundleid1=bundleid1, bundleid2=bundleid2)
        ...
        if ktype == "xd" or ktype == "dx":
            ...
            if ktype == "xx": #config["pspec"]["combine_mf"] and ktype == "xx":
        ...
        if ktype == "mf":
            file_plmstack = dir_plm + f"{gcmode}lmstack{qe}_xx.npz"
            plmstack = np.load(file_plmstack)
            mf = process_mf(seed, plm, plmstack, config["pspec"][f"mfsplit{mapnum}"], "xx", gcmode)
    ...
    if bundleid1 is None: 
        ...
        kmaps_dir = kmaps_dir+'bundle%sXbundle%s/'%(bundleid1, bundleid2)
    pathlib.Path(kmaps_dir).mkdir(parents=True, exist_ok=True)

    #hp.write_alm(kmaps_dir+f"kmap{ktype}_{qe}_{seed}_{mapnum}.alm", kmv, overwrite=True)
...
def cleanup(config, seed, nber_bundles=None):
    ...
    if nber_bundles is None:
        ...
        for bundleid in range(nber_bundles):
            [print(f) for f in Path(dir_tmp+'bundle%sXbundle%s'%(bundleid, bundleid)).glob(f"kmap*_{seed}_1.fits")]
            [print(f) for f in Path(dir_tmp+'bundle%sXbundle%s'%(bundleid, bundleid)).glob(f"kmap*_{seed}_2.fits")]
            [f.unlink() for f in Path(dir_tmp+'bundle%sXbundle%s'%(bundleid, bundleid)).glob(f"kmap*_{seed}_1.fits")]
            [f.unlink() for f in Path(dir_tmp+'bundle%sXbundle%s'%(bundleid, bundleid)).glob(f"kmap*_{seed}_2.fits")]
...
if __name__ == "__main__":
    ...
    if args.xx:
        ...
        for bundleid in range(nber_bundles):
            get_kmap(config, seed, ktype="xx", mapnum=1, bundleid1=bundleid)
            get_kmap(config, seed, ktype="xx", mapnum=2, bundleid1=bundleid)
        ...
        for bundleid1 in range(nber_bundles):
            for bundleid2 in range(nber_bundles):
                if bundleid1>bundleid2: 
                    ...
                    compute_cls(config, seed, ktype='xxxx', bundleid1=bundleid1, bundleid2=bundleid2)
    ...
    if args.N0:
        ...
        for bundleid in range(nber_bundles):
            get_kmap(config, seed, ktype="xy", mapnum=1, bundleid1=bundleid)
            get_kmap(config, seed, ktype="xy", mapnum=2, bundleid1=bundleid)
            get_kmap(config, seed, ktype="yx", mapnum=2, bundleid1=bundleid)
    ...
    if args.RDN0:
        ...
        for bundleid in range(nber_bundles):
            get_kmap(config, seed, ktype="xd", mapnum=1, bundleid1=bundleid)
            get_kmap(config, seed, ktype="xd", mapnum=2, bundleid1=bundleid)
            get_kmap(config, seed, ktype="dx", mapnum=1, bundleid1=bundleid)
            get_kmap(config, seed, ktype="dx", mapnum=2, bundleid1=bundleid)
    ...
    if args.curlRDN0:
        ...
        for bundleid in range(nber_bundles):
            get_kmap(config, seed, ktype="wd", mapnum=1, bundleid1=bundleid)
            get_kmap(config, seed, ktype="wd", mapnum=2, bundleid1=bundleid)
            get_kmap(config, seed, ktype="dw", mapnum=1, bundleid1=bundleid)
            get_kmap(config, seed, ktype="dw", mapnum=2, bundleid1=bundleid)
    ...
    if args.N1:
        for bundleid in range(nber_bundles):
            get_kmap(config, seed, ktype="ab", mapnum=1, bundleid1=bundleid)
            get_kmap(config, seed, ktype="ab", mapnum=2, bundleid1=bundleid)
            get_kmap(config, seed, ktype="ba", mapnum=2, bundleid1=bundleid)
    ...
    if args.MF:
        ...
        for bundleid in range(nber_bundles):
            get_kmap(config, seed, ktype="mf", mapnum=1, bundleid1=bundleid)
            get_kmap(config, seed, ktype="mf", mapnum=2, bundleid1=bundleid)
