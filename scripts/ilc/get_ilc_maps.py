...
def get_map_locations(fieldname, nuarr, data_or_sim, bundle_index, sim_index, source_template_str = '6.0to5000.0mJy', scott=True, mask_ilc_map = True):
    ...
    if scott:
        ...
        op_fd = '/data/gpfs/projects/punim1922/summerfield_lensing/ilc/maps/len/'
               
        if fieldname == 'summer_a':
            data_fd = '/data/gpfs/projects/punim1922/summerfield_lensing/data/coadds/full/maps/summer-a/'  
        ...
        if fieldname == 'summer_b':
            data_fd = '/data/gpfs/projects/punim1922/summerfield_lensing/data/coadds/full/maps/summer-b/'  
        ...
        if fieldname == 'summer_c':
            data_fd = '/data/gpfs/projects/punim1922/summerfield_lensing/data/coadds/full/maps/summer-c/'  
        ...
        for bundle_index in bundle_arr:
            ...
            for keyname in reqd_ilc_keynames: #loop over the ILCs
                ...
                for nu in mapfname_dic:
                    if data_or_sim == 'data':
                        print('%s GHz data'%(nu))
                        mapframe = read_map( mapfname_dic[nu], band='%sGHz'%(nu))
                        maps.RemoveWeights( mapframe, zero_nans = True )
                        curr_hmap = np.asarray(mapframe['T'])
                        curr_hmap = curr_hmap * 1e3 
                    elif data_or_sim == 'sims':
                        ...
                        #curr_noise_map[curr_noise_map < -1.e10] = 0
                        #curr_noise_map[curr_noise_map > 1.e10] = 0
                        
                        curr_hmap += curr_noise_map*1e3 
                    
                    #remove the source template for data
                    if data_or_sim == 'data': 
                        curr_psource_template = np.asarray(read_map_frame(psource_template_fname_dic[nu])['T'])
                        curr_hmap = curr_hmap - curr_psource_template

                    if perform_inpainting: 
                        curr_hmap = inpaiting_wrapper(curr_hmap, inp_ra_list, inp_dec_list, inp_rad_am_list, parent_sigma_dic)
                        if sim_index==0:
                            H.write_map('inpainted_map_%sghz.fits'%(nu), curr_hmap, overwrite=True)

                    #if mask_ilc_map:
                        #curr_map_alm = H.map2alm(curr_hmap*point_source_apod_mask, lmax = lmax)
                    #else:
